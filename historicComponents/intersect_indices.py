...
output_csv = os.path.join(data_dir, 'nasdaq_and_sp500_quarterly.csv')
...
# 3. Perform intersection (NDX AND S&P500)
results = []
for idx in range(len(df_ndx_filtered)):
    ...
    # Intersection: Tickers in both NDX and S&P500
    intersect = ndx_tkrs.intersection(sp_tkrs)
    intersect_sorted = sorted(list(intersect))
    ...
    results.append({
        ...
        'Constituents': ','.join(intersect_sorted)
    })

df_results = pd.DataFrame(results)
df_results.to_csv(output_csv, index=False)
print(f"\nSuccessfully generated index intersection and saved to: {output_csv}")

# Display summary of the last 8 quarters
print("\nSummary of the last 8 quarters:")
cols_to_print = ['Quarter', 'Start_Date', 'End_Date', 'Nasdaq_Count', 'SP500_Count', 'Intersection_Count']
...
try:
    ...
    ax.plot(df_results['Quarter'], df_results['Intersection_Count'], marker='o', color='#00b4d8', linewidth=2.5, markersize=4, label='S&P 500 ∩ NASDAQ-100')

    # Labels and title
    ax.set_title('Number of Companies in S&P 500 and NASDAQ-100 Intersection by Quarter', fontsize=14, fontweight='bold', pad=15)
    ...
    y_min = df_results['Intersection_Count'].min() - 3
    y_max = df_results['Intersection_Count'].max() + 3
    ...
    plot_img_path = os.path.join(data_dir, 'nasdaq_and_sp500_count_chart.png')
    plt.savefig(plot_img_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {plot_img_path}")
    ...
    if os.path.exists(artifact_dir):
        import shutil
        shutil.copy(plot_img_path, os.path.join(artifact_dir, 'nasdaq_and_sp500_count_chart.png'))
